      ...
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[0][1]).toBe(
            "Submitted a primary block (GamePass:1) successfully to GamePass's daemon instance(s)"
          );
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][1]).toBe(
            "Requested template from primary chain (GamePass:2) via RPC polling"
          );
          done();
        }
      }
      ...
      if (type !== "debug") {
        ...
        if (response.length === 3) {
          ...
          expect(response[2][1]).toBe(
            "Requested template from primary chain (GamePass:1) via RPC polling"
          );
          done();
        }
      }
