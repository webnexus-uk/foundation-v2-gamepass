...
const Stratum = function (logger, config, configMain) {
  ...
  this.handleStratum = function (callback) {
    ...
    _this.stratum.on("pool.share", (shareData, shareValid) => {
      ...
      if (shareValid) {
        ...
        const text = _this.text.stratumSharesText1(
        ...
        const address = shareData.addrPrimary.split(".")[0];
        const text = _this.text.stratumSharesText2(
          shareData.error,
          address,
          shareData.ip
        );
  ...
  this.outputStratum = function () {
    ...
    const output = [
      ...
      _this.text.startingMessageText3(
        _this.config.settings.testnet ? "Testnet" : "Mainnet"
      ),
      _this.text.startingMessageText4(
        _this.stratum.statistics.ports.join(", ")
      ),
      _this.text.startingMessageText5(
        _this.stratum.statistics.feePercentage * 100
      ),
      _this.text.startingMessageText6(
        _this.stratum.manager.currentJob.rpcData.height
      ),
